...
const dbPath =
  process.env.DATABASE_URL || path.resolve(process.cwd(), 'sqlite.db')

declare global {
  var __sqlite_db: ReturnType<typeof drizzle<typeof schema>> | undefined
}
...
function initDb() {
  ...
  // Ensure products table exists automatically
  sqlite.exec(`
  ...
  if (countRow.count === 0) {
    const insert = sqlite.prepare(`
      INSERT INTO products (id, name, slug, description, category, price, rating, stock, image_url, featured)
      VALUES (@id, @name, @slug, @description, @category, @price, @rating, @stock, @imageUrl, @featured)
    `)
    ...
    const seedMany = sqlite.transaction((items) => {
      for (const item of items) {
        insert.run(item)
      }
    })

    seedMany(initialProducts)
  }

  return drizzle(sqlite, { schema })
}

export const db = globalThis.__sqlite_db ?? (globalThis.__sqlite_db = initDb())
